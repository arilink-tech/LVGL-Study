...
static void ui_Screen1_screen_init(void)
{
    ...
    lv_obj_t* label3 = lv_label_create(ui_Screen1);
    //lv_label_set_text(label, "LVGL Label Study");
    lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 0);//label对象放置在顶部中间
    lv_obj_align(label3, LV_ALIGN_TOP_MID, 0, 30);//style对象放置在顶部中间
